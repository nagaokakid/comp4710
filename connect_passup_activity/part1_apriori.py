...
def apriori_pre():
    ...
    pass_up_df["Route Destination"] = pass_up_df["Route Destination"].apply(str)
    pass_up_df['stop_id'] = pass_up_df["Route Number"]+" "+pass_up_df["stop_id"]+" "+pass_up_df["Route Destination"]
    print(len(pass_up_df['stop_id']))

    for x in range(len(pass_up_df['stop_id'])):
        print(pass_up_df["stop_id"][x])
    ...
    result = df.to_csv("pass_up_apriori_result_pre.csv")
